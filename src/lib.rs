...
pub struct Remover<'a>(&'a mut Vec<Building>, HashMap<i32, i32>, Vec<i32>, Vec<i32>, &'a mut u32);
impl<'a> Remover<'a> {
            ...
            with_rounding(
                ...
                || {
                    let _ = match &dump.locale {
                        None => GLOBAL_SERIALIZATION_LOCALE.set(Locale::cn),
                        Some(s) => {
                            GLOBAL_SERIALIZATION_LOCALE.set(Locale::try_from_user_string(s)?)
                        }
                    };
                    let mut input = input()?;
                    ...
                    for cnt in rm.3.iter() {
                        ...
                        if belt_input ^ belt_output {
                            ...
                            if let Belt(Some(belt)) = belt {
                                ...
                                {
                                    if dump.verbose {
                                        println!("已将{bidx}节点识别为数量{idx}的{}{}端", belt.label, if belt_input {"输入"} else {"输出"})
                                    }
                                } else {
                        ...
                        .flat_map(|x| x.1.into_iter().map(move |y| (x.0, y.0)))
                    {
                        let Some(mut iter1) = sorter_labels.remove(&label) else {
                            eprintln!(
                                "在sorter_labels中找不到{label}，sorter_labels={sorter_labels:?}"
                            );
                            continue;
                        };
